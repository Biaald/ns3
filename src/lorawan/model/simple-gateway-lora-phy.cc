                                   ...
                                   double frequencyMHz)
{
    ...
    /*for(int l=0; l<L; l++) {
        fading = Rayleigh();
        antennaPower[l] = rxPower * fading;
    }*/
   
    //double combinedRxPowerDbm = ApplyCombining(rxPowerDbm);

    // Add the event to the LoraInterferenceHelper
    // recebe apenas 1 sinal por vez, entao o vetor de eventos do interference helper tem no maximo 1 elemento, e o evento é atualizado a cada nova recepcao
    Ptr<LoraInterferenceHelper::Event> event;
    //sinal útil que o helper vai usar para calcular a interferencia, e que vai ser passado para EndReceive para identificar o sinal que esta sendo recebido
